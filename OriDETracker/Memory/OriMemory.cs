...
namespace OriDE.Memory
{
	public partial class OriMemory {
		...
		public OriMemory()
		{
			gameWorld = new ProgramPointer(this, MemPointer.GameWorld) {
				AutoDeref = false
			};
			gameplayCamera = new ProgramPointer(this, MemPointer.GameplayCamera) {
				AutoDeref = false
			};
			worldEvents = new ProgramPointer(this, MemPointer.WorldEvents) {
				AutoDeref = false
			};
			seinCharacter = new ProgramPointer(this, MemPointer.SeinCharacter) {
				AutoDeref = false
			};
			scenesManager = new ProgramPointer(this, MemPointer.ScenesManager) {
				AutoDeref = false
			};
			gameStateMachine = new ProgramPointer(this, MemPointer.GameStateMachine) {
				AutoDeref = false
			};
			rainbowDash = new ProgramPointer(this, MemPointer.RainbowDash) {
				AutoDeref = false
			};
			gameController = new ProgramPointer(this, MemPointer.GameController) {
				AutoDeref = false
			};
			tas = new ProgramPointer(this, MemPointer.TAS) {
				AutoDeref = false
			};
			coreInput = new ProgramPointer(this, MemPointer.Input) {
				AutoDeref = false
			};
		}
		...
		{
			if (coreInput.Value == IntPtr.Zero)
			{
				return new PointF();
			}
		...
		public void SetSkills(bool enable, params Skill[] skills)
		{
			...
			for (int i = 0; i < skills.Length; i++)
			{
				...
				{
				case Skill.Sein:
					SetAbility("Spirit Flame", enable);
					break;
				case Skill.WallJump:
					SetAbility("Wall Jump", enable);
					break;
				case Skill.ChargeFlame:
					SetAbility("Charge Flame", enable);
					break;
				case Skill.Dash:
					SetAbility("Dash", enable);
					break;
				case Skill.DoubleJump:
					SetAbility("Double Jump", enable);
					break;
				case Skill.Bash:
					SetAbility("Bash", enable);
					break;
				case Skill.Stomp:
					SetAbility("Stomp", enable);
					break;
				case Skill.Glide:
					SetAbility("Glide", enable);
					break;
				case Skill.Climb:
					SetAbility("Climb", enable);
					break;
				case Skill.ChargeJump:
					SetAbility("Charge Jump", enable);
					break;
				case Skill.Grenade:
					SetAbility("Light Grenade", enable);
					break;
				}
			}
		}
		...
		public string GetPointer(string name)
		{
			...
			{
			case "GameWorld":
				return gameWorld.Value.ToString("X");
			case "GameplayCamera":
				return gameplayCamera.Value.ToString("X");
			case "WorldEvents":
				return worldEvents.Value.ToString("X");
			case "SeinCharacter":
				return seinCharacter.Value.ToString("X");
			case "ScenesManager":
				return scenesManager.Value.ToString("X");
			case "GameStateMachine":
				return gameStateMachine.Value.ToString("X");
			case "RainbowDash":
				return rainbowDash.Value.ToString("X");
			}
			return string.Empty;
		}
	...
	public class ProgramPointer {
		...
		public OriMemory Memory {
			get;
			set;
		}
		public MemPointer Name {
			get;
			set;
		}
		public MemVersion Version {
			get;
			set;
		}
		public bool AutoDeref {
			get;
			set;
		}
		private int lastID;
		...
		private IntPtr GetVersionedFunctionPointer()
		{
			foreach (MemVersion version in Enum.GetValues(typeof(MemVersion)))
			{
				...
				if (!funcPatterns.TryGetValue(version, out patterns))
				{
					continue;
				}

				string pattern = null;
				if (!patterns.TryGetValue(Name, out pattern))
				{
					continue;
				}
